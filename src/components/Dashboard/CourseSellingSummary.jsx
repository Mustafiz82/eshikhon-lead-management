...
const CourseSellingSummary = ({ state }) => {
  ...
  const { data: summary } = useFetch(
    `dashboard/courseSellingSummary?startDate=${state[0].startDate}&endDate=${state[0].endDate}&${query}`,
  );
  ...
  const searchedData = (summary || []).filter((item) =>
    item.courseName.toLowerCase().includes(searchText?.toLowerCase() || ""),
  );

  const sortedData = sortCourses(
    searchedData,
    selectedOption,
    sortKey,
    sortOrder,
        ...
        <div className="flex p-6 pb-0 justify-between items-center mb-4">
          ...
          <div className="flex  gap-5 flex-1 justify-end">
            ...
            <select
              ...
              className="select static! focus-within:ring-0 focus-within:outline-0 focus-within:border-blue-600 pl-5  "
            >
            ...
            sortedData?.map((item, index) => (
              ...
              >
                ...
                <p className="text-center ">
                  {" "}
                  <CountUp className="" end={item.totalSales} />{" "}
                </p>
                <p className="text-center ">
                  {" "}
                  <CountUp className="" end={item.totalDue} />{" "}
                </p>
                <p className="col-span-2 w-[200px] mx-auto">
                  {" "}
                  <ProgressBar
                    assigned={item?.online?.assigned}
                    enrolled={item?.online?.enrolled}
                  />
                </p>
                <p className="col-span-2 w-[200px] mx-auto">
                  {" "}
                  <ProgressBar
                    assigned={item?.offline?.assigned}
                    enrolled={item?.offline?.enrolled}
                  />
                </p>
                <p className="col-span-2 w-[200px] mx-auto">
                  {" "}
                  <ProgressBar
                    assigned={item?.total?.assigned}
                    enrolled={item?.total?.enrolled}
                  />
                </p>
              </div>
            ))
...
const ProgressBar = ({ assigned, enrolled }) => {
  ...
  return (
    <div className="flex flex-col mt-2 h-full">
      ...
      <div
        className="relative flex w-full h-4 bg-black/20 rounded-full overflow-hidden"
        role="progressbar"
      >
        {/* The Progress Fill */}
        <div
