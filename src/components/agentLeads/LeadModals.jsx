  ...
  const LeadModals = ({
    ...
    const [modelStatus, setModelStatus] = useState(
      selectedLead?.leadStatus || "Pending",
    );
    const [InterstedSeminarStatus, setInterstedSeminarStatus] = useState(
      selectedLead?.interstedSeminar || "None",
    );
    ...
    const searchSuggestions =
      searchInput?.trim() && isDropdownOpen
        ? course.filter((item) =>
    ...
    const handleSaveChanges = async () => {
      ...
      if (orderNumber) {
        ...
        if (
          formatForWhatsApp(customerPhone) !==
            formatForWhatsApp(selectedLead?.phone) &&
          modelStatus !== "Enrolled with Other Number" &&
          user?.role == "user"
        ) {
          return setError(
            "The order number must match the phone number associated with this lead ",
          );
        } else {
          setError("");
        }
      ...
      console.log(
        notes
          .filter((item) => item?.status == "unsaved")
          .map(({ text, by }) => ({ text, by })),
      );
    ...
    useEffect(() => {
      if (selectedLead) {
        ...
        setSearchInput(
          selectedLead?.interstedCourse ?? selectedLead?.interstedCourse ?? "",
        );
        ...
        const selectedCourse = course.find(
          (courseItem) =>
            courseItem.name === selectedLead?.interstedCourse &&
            courseItem.type === selectedLead?.interstedCourseType,
        );
        ...
        setLocalHistory(selectedLead.history);
      }
    }, [selectedLead]);
        ...
        <div className="fixed inset-0 !z-9999 bg-black/40 flex items-center justify-center">
          ...
          >
            ...
            <div className="space-y-6 max-h-[450px] [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]  overflow-y-scroll">
              ...
              {/* SECTION: COURSE INFO */}
              <div>
                <h4 className="text-xs font-bold text-white/80 mb-1">
                  Course Info
                </h4>
                <div className="grid grid-cols-[130px_1fr] gap-y-1 text-sm">
                  <div className="text-white/50">Interested Course</div>
                  <div
                    title={selectedLead.interstedCourse}
                    className="font-medium line-clamp-1 "
                  >
                    {selectedLead.interstedCourse}
                  </div>

                  <div className="text-white/50">Course Type</div>
                  <div className="font-medium">
                    {selectedLead.interstedCourseType}
                  </div>
                  ...
                  <div className="flex items-center justify-between font-medium">
                    ...
                    {leadSource !== (selectedLead?.leadSource || "") && (
                      <span className="ml-2 text-yellow-500 font-semibold text-xs">
                        (Unsaved)
                      </span>
                    )}
              ...
              <div>
                ...
                <div className="grid grid-cols-[130px_1fr] gap-y-1 text-sm">
                  ...
                  <div className="font-medium">
                    {formateDate(selectedLead.assignDate)}
                  </div>

                  <div className="text-white/50">Follow-Up Date</div>
                  <div className="font-medium">
                    {selectedLead.followUpDate
                      ? formateDate(selectedLead.followUpDate)
                      : "N/A"}
                  </div>
                  ...
                  <div className="font-medium">
                    {selectedLead.nextEstimatedPaymentDate
                      ? formateDate(selectedLead.nextEstimatedPaymentDate)
                      : "N/A"}
                  </div>
                  ...
                  <div className="font-medium">
                    {selectedLead.lastContacted
                      ? formateDate(selectedLead.lastContacted)
                      : "N/A"}
                  </div>
                </div>
              </div>
              ...
              {selectedLead?.questions &&
                Object.keys(selectedLead.questions).length > 0 && (
                  <div>
                    <h4 className="text-xs font-bold text-white/80 mb-1">
                      Questions
                    </h4>
            ...
            <div className="space-y-2 flex flex-col text-sm">
              ...
              <div className="space-y-2 h-[250px]   overflow-y-auto">
                {notes?.map((note, i) => (
                  <div
                    key={i}
                    className="border border-base-300 mt-2 p-2 bg-base-200 rounded relative"
                  >
                    ...
                    <span>{
                      (note.date || note.createdAt) && formateDate(note.date || note.createdAt) + ` • ${note.by}`
                    }  </span>
                      {note?.status == "unsaved" && (
                ...
                {notes?.length === 0 && (
                  <p className="text-xs text-center mt-20  text-base-content/60">
                    No notes yet.
                  </p>
                )}
              </div>
              ...
              <form className="mt-auto" onSubmit={handleAddNote}>
                ...
                ></textarea>
                <button className="btn w-full mt-2 bg-blue-600 btn-primary ">
                  Add Note
                </button>
              </form>
            </div>
            ...
            <div className="flex flex-col gap-2 ">
              ...
              <div className="mt-2 relative">
                <input
                  ...
                  disabled={
                    user?.role === "user" &&
                    selectedLead.interstedCourse &&
                    selectedLead.leadStatus === "Enrolled"
                  }
                  value={searchInput}
                ...
                {/* Badge displays type derived directly from selected ID */}
                {selectedCourseType && (
                ...
                {/* Suggestions list derived directly from searchInput */}
              ...
              <button
                ...
                title={
                  selectedLead?.isLocked
                    ? "Lead is Locked . Contact Admin to modify the leads "
                    : ""
                }
